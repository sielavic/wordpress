jQuery(document).ready(function ($) {
    ...
    $('#add_feedback input, #add_feedback textarea').on('blur', function () {
        ...
        $('.error-name,.error-email,.error-additional,.error-phone, .error-birthday,.message-success').remove();
        ...
        success: function (request, xhr, status, error) {
            ...
            if (request.success === true) {
                ...
                $.each(request.data, function (key, val) {
                    $('.ex_' + key).addClass('error');
                    $('.ex_' + key).before('<span class="error-' + key + '">' + val + '</span>');
                });
